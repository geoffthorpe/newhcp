...
#   is;
#       setup-global setup-local start-services
...
#   The target list is;
#       setup-local start-services
do_core_start()
{
	wrapper start setup-local start-services -- "$@"
}
...
do_core_setup()
{
	wrapper run_fg setup-global -- "$@"
}

# do_core_fg()
# - runs a foregrounded task/tool. The first argument is the workload name.
do_core_fg()
{
	if [[ $# -eq 0 ]]; then
...
do_normal_start()
{
	wrapper start start-attester setup-local start-services -- "$@"
}
do_normal_setup()
{
	wrapper run_fg start-attester setup-global -- "$@"
}
do_normal_fg()
{
	if [[ $# -eq 0 ]]; then

...
def attest_verify(quote_file):
	...
	sub = subprocess.run(["/install-safeboot/sbin/tpm2-attest", "verify", quote_file ],
	...
	# this ekhash.
	sub = subprocess.run(["/install-safeboot/sbin/attest-verify", "verify", str(quote_valid)],
	...
	result = subprocess.run(["/install-safeboot/sbin/tpm2-attest", "seal", quote_file, ],
		input=response,
		capture_output=True
	)

...
os.environ['KRB5_KDC_PROFILE'] = f"{kdcstate}/etc/kdc.conf"

def run_subprocess(args):
	mylog(f"running: {args}")
...
if cmd == "add":
	...
	for p in principals_args:
		res['stdout'] += run_subprocess([
			'kadmin.local', '-q', f"addprinc -randkey {p}"])
	print(json.dumps(res))

elif cmd == "get":
	# TODO: implement something (is this 'list' or 'get'??)
	sys.exit(1)

elif cmd == 'del':
	for p in principals_args:
		res['stdout'] += run_subprocess([
			'kadmin.local', '-q', f"delprinc {p}"])
	print(json.dumps(res))
	...
	with tempfile.TemporaryDirectory() as td:
		tf = f"{td}/kt"
		for p in principals_args:
			run_subprocess(['kadmin.local', 'ktadd', '-k', tf, p])
		res['stdout'] = run_subprocess(['base64', '--wrap=0', tf])
